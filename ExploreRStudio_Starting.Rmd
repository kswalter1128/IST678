...
temps_f <- c(57,63,62,72,58,63,57)
```
...
Let’s compare youinr first city to two others. Look up two new cities and find the temperature (in Fahrenheit) of those cities for the next seven days. Along with the first city you chose, create a List city temperature. Name each item in the list after 

```{r}
# Create your list here
moscow_tepms_f <- c(44,48,46,44,45,47,48)
capetown_temps_f <- c(73,72,68,64,63,54,64)
Towns <- list(Tokyo = temps_f,
              Moscow = moscow_tepms_f,
              Capetow = capetown_temps_f)
```
...
WeatherDf <- as.data.frame(Towns)
boxplot(WeatherDf)
require(ggplot2)
require(reshape)
m_weather_df <- melt(WeatherDf)
ggplot(m_weather_df,aes(x=variable, y=value))+geom_boxplot()+xlab("City")+ylab("Temp in F")+ggtitle("Weather forecast next 7 days")
```
