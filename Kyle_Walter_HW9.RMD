...
ST687	– Support	Vector	Machines	Lab
...
g <- ggplot(compTable, aes(x=Temp, y = Wind, size = Residual))+geom_point()+ggtitle("ksvm")
g
```
...
g2 <- ggplot(compTable2, aes(x=Temp, y = Wind, size = Residual))+geom_point()+ggtitle("SVM")
g3 <- ggplot(compTable3, aes(x=Temp, y = Wind, size = Residual))+geom_point()+ggtitle("LM")
```
...
```{r}
aq$goodOzone <- ifelse(aq$Ozone<mean(aq$Ozone),0,1)#Creates a column in the data set that tests if the Ozone is good or bad. Good = 0 bad = 1
```
...
goodOCompTable$Residual <- goodOCompTable$aqTest.goodOzone - goodOCompTable$goodOzonePred #creates the residual between what the value was vs what was predicted
...
goodOCompTable$Discrete <- as.factor(ifelse(goodOCompTable$goodOzonePredRound==1,"Bad","Good"))
g4 <- ggplot(goodOCompTable, aes(x=Temp, y=Wind, size=Residual, shape = Discrete, color = goodOzone))+geom_point()+ggtitle("KSVM")
g4
```
...
aqTrain2 <- aqTrain
aqTrain2$goodOzone <- ifelse(aqTrain2$goodOzone==1,"Bad","Good")
aqTrain2$goodOzone <- as.factor(aqTrain2$goodOzone)
aqTest2 <- aqTest
aqTest2$goodOzone <- ifelse(aqTest2$goodOzone==1, "Bad","Good")
aqTest2$goodOzone <- as.factor(aqTest2$goodOzone)
goodModel3 <- naiveBayes(goodOzone~., data = aqTrain2)
...
goodOCompTable2$Residual <- goodOCompTable2$aqTest.goodOzone - goodOCompTable2$goodOzonePred2
colnames(goodOCompTable2) <- c("goodOzone","goodOzonPred","goodOzonePredRound","Wind","Temp","Residual")
goodOCompTable2$Discrete <- as.factor(ifelse(goodOCompTable2$goodOzonePredRound==1,"Bad","Good"))
goodOCompTable3 <- data.frame(aqTest2$goodOzone, goodOzonePred3, goodOzonePredRound, aqTest2$Wind, aqTest2$Temp)
goodOCompTable3$Residual <- ifelse(goodOCompTable3$aqTest2.goodOzone == goodOCompTable3$goodOzonePred3,0,1)
colnames(goodOCompTable3) <- c("goodOzone","goodOzonPred","goodOzonePredRound","Wind","Temp","Residual")
g5 <- ggplot(goodOCompTable2, aes(x=Temp, y=Wind, size=Residual, shape = Discrete, color=goodOzone))+geom_point()+ggtitle("SVM")
g6 <- ggplot(goodOCompTable3, aes(x=Temp, y=Wind, size=Residual, Shap = goodOzonePred, color = goodOzone))+geom_point()+ggtitle("Naieve Bayes")
```
...
While the models get significantly better going from trying to predict the actual Ozone Value to predicting whether it was good or bad based on it's realtive measurement to the average. The Naieve Bayes model seems to have the best outcomes missing classifications on <5 outcomes on various runs, while the other models seem to incorrectly classify a little less than 10 observations.